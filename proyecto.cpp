...
int main() {
	...
	while (intentos > 0) {
		...
		if (posX == columnas - 1 && posY == filas - 1) {
			std::cout << "¡Llegaste a la meta! ¡Ganaste!" << std::endl;
			break;
		}
		
		// Limpiar la pantalla antes de mostrar la siguiente iteración
		clearScreen();
	}
	...
	// Mostrar los resultados de las operaciones almacenados en la meta
	gotoxy(0, filas * 2 + 2);
	std::cout << "Resultados de las operaciones en la meta: ";
